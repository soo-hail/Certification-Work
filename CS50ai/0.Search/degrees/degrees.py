...
def shortest_path(source, target):
    ...
    while not q.empty():
        ...
        for movie_id in people[id]['movies']: 
            for star in movies[movie_id]['stars']:  # CO-ACTORS
                if star not in visited:
                    new_node = Node(star, curr_node, movie_id)  # Create new node for each co-actor
                    q.add(new_node)  # Add the new node to the queue
